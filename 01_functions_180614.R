...
switch1 <- function(swi) {
  ...
  swi3 <-
    anti_join(swi, swi2, by = c("count", "roost", "year", "swyr")) %>%
    arrange(roost)
  ...
  swi <- swi %>%
    mutate(roost = ifelse(roost == 0, seq(
      max(swi$roost) + 1, max(swi$roost) + num, 1
    ), roost)) %>%
    arrange(roost) %>%
...
switch2 <- function(swi) {
  ...
  if (dim(swi2)[1] == 0) {
    ...
    swi3 <-
      anti_join(swi, swi2, by = c("roost", "count", "year", "orgroost", "swyr"))
...
# something is wrong with this fct
# applying it gives us a GAM that is almost identical to truth
accessibility <- function(obs,
                          a = 0,
                          b = 1,
                          c = 1, # this needs to be compared / discussed
                          d = 4) {
  ...
  obs <- obs %>% 
    rowwise(.) %>% 
    mutate(acc = ((slope * count) + int) ^ c)
  
  lth <- length(obs$count) / d
  
  perc <- runif(1, lth * 2, lth * 3) # calculates the amount of roosts to be sampled (slightly random)
  
  acc <- obs[sample(nrow(obs), perc, replace = F, prob = obs$acc), ]
  
  return(acc)
}

motivation <- function(all){
  
  # create tibble with the original no of roosts, drop swyr and orgroost
  orig <- all %>%
    select(count, roost, year) %>%
    dplyr::filter(roost <= z)
  
  # create tibble with only the new roosts
  new <- all %>%
    dplyr::filter(roost > z) %>%
    select(count, year, orgroost, swyr) %>%
    rename(roost = orgroost)
  
  # match them together
  both <- left_join(orig, new, by = c("year", "roost")) %>%
    select(-count.y) %>%
    rename(count = count.x)
  
  # give the first year a swyr of 1 (otherwise na.locf won't work)
  both <- both %>%
    group_by(roost) %>%
    mutate(swyr = ifelse(year == 1, 1, swyr))
  
  # copy previous row value for swyr NAs
  both <- both %>%
    group_by(roost) %>%
    arrange(year) %>%
    do(zoo::na.locf(.))
  
  # drop any roosts after swyr > 2
  obs <- both %>%
    group_by(roost) %>%
    dplyr::filter(swyr <= 2)
...
invest <- function() {
  # create index of continous survey length (in years) per roost
  # but hooow
  # oh god this might have to become a loop....
  obs <- acast(obs, roost ~ year, value.var = "count", fill = 0)
  obs <- melt(obs, value.name = "count") %>%
    rename(roost = Var1, year = Var2)
  
  
  
  # create linear relationship between continous survey length and investment
...
start_stop_monitoring <- function(mon) {
  ...
  sam <- mon %>% 
    group_by(roost) %>% 
    sample_frac(., size = runif(1, 0.65, 0.85)) %>% 
    arrange(roost, year)
...
gam_func <- function(sp, dfvec = c(6)) {
  gam <- gam(
    count ~ as.factor(roost) + s(year, fx = TRUE, k = dfvec + 1),
...
index_func <- function(sp, gam, dfvec = c(6)) {
  if (length(sp$count[is.na(sp$count)]) > 0)
...
boot_func <- function(sp, defree = 6) {
  ...
  dr_roost <-
    rep(1:Nroosts, as.vector(unlist(lapply(elements, length))))
  ...
  dr_gam <-
    gam(
      count ~ s(year, fx = TRUE, k = defree + 1) + as.factor(roost),
      family =
        poisson(link = log),
      data = data_resample
    )	#
  ...
  dr_new <-
    expand.grid(year = year_base + 1:Nyears, roost = sort(unique(dr_roost)))
  
  # predict from model for all years
  result <-
    predict.gam(dr_gam, newdata = dr_new, type = "terms")
...
ci_func <- function(index_matrix, conf = 0.95) {
  ...
  if (abs((Nrep + 1) * alp - round((Nrep + 1) * alp)) > 1e-05)
    stop(
      "need to discard rows of index.matrix, or change conf,

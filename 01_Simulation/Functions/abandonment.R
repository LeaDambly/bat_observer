...
abandon <- function(pop, p = 0.01) {

  # create new tibble with populations that have abandoned their roost
  pop2 <- pop %>%
    dplyr::filter(roost <= z & count > 0 & is.na(reocc)) %>%
  ...
  pop3 <- anti_join(pop, pop2, by = c("count", "roost", "year", "abyr")) %>%
    arrange(roost) %>%
    mutate(abyr = ifelse((roost < z & count == 0), abyr + 1, abyr))
  ...
  pop <- pop %>%
    ...
    # add another year to abyr for prev. switched roosts
    mutate(abyr = ifelse((roost > z & count > 0 & !is.na(abyr)), abyr + 1, abyr)) %>%
    # give newly switched roosts their first abyr
    mutate(abyr = ifelse((roost > z & count > 0 & is.na(abyr)), 1, abyr)) %>%
    mutate(reocc = NA)

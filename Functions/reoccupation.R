...
#
# It returns pop: a tibble with columns count, roost, year, swyr, orgroost
...
reoccupy <- function(pop, p2 = 0.7) {
  ...
  pop2 <- pop %>%
    dplyr::filter(!is.na(orgroost) & swyr >= 1) %>%
    sample_frac(p2)
  ...
  if (dim(pop2)[1] == 0) {
    ...
    pop3 <- anti_join(pop, pop2, by = c("roost", "count", "year", "orgroost", "swyr"))
